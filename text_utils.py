...
class RenderFont(object):
    ...
    def __init__(self, data_dir="data"):
        ...
        self.text_source = TextSource(
            ...
            fn=osp.join(data_dir, "newsgroup/newsgroup_th.txt"),
        )
    ...
    def render_curved(self, font, word_text):
        ...
        first_bounds.y = rect.y - first_bounds.y
        first_ch_bb = np.array(first_bounds)

        ### render chars to the left and right:
        last_rect = rect
        last_char_rect = last_rect
        ...
        for i in range(wl):
            ...
            if ch not in up_vowel and ch not in down_vowel:
                last_char_rect = newrect
    ...
    def fen__render_curved(self, font, word_text):
        ...
        curve_ = []
        count = 0
        for num, char in enumerate(word_text):

            if char not in up_vowel and char not in down_vowel:

                curve_.append(count)
                count += 1
            else:
                curve_.append(count)
        curve_char = len(list(set(curve_)))

        # baseline state
        mid_idx = wl // 2
        BS = self.baselinestate.get_sample()
        curve = [BS["curve"](i - mid_idx) for i in range(curve_char)]
        curve[mid_idx] = -np.sum(curve) / (wl - 1)
        rots = [
            -int(math.degrees(math.atan(BS["diff"](i - mid_idx) / (font.size / 2))))
            for i in range(curve_char)
        ]
        curve = [curve[i] for i in curve_]
        rots = [rots[i] for i in curve_]
        bbs = []
        ...
        for i in range(wl):
            ...
            newrect.y = last_rect.y
            if i > mid_idx:
                if last_ch in up_vowel and ch in up_vowel:
                    newrect.topleft = (
                        last_char_rect.bottomright[0] + 2,
                        last_rect.topleft[1] - newrect.height,
                    )
                ...
                elif last_ch in up_vowel or last_ch in down_vowel:
                    if ch in up_vowel and last_ch not in down_vowel:
                        newrect.topright = (
                            last_char_rect.topleft[0] + newrect.width + 2,
                            last_char_rect.topleft[1] - 2,
                        )
                    else:
                        newrect.topright = (
                            last_char_rect.topleft[0] + 2,
                            last_char_rect.topleft[1],
                        )
                elif ch in up_vowel:
                    if next_ch in up_vowel:
                        next_rect = font.get_rect(next_ch)
                        newrect.topright = (
                            last_char_rect.topleft[0] + newrect.width + 2,
                            last_char_rect.topleft[1] - next_rect.height - 2,
                        )
                    else:
                        newrect.topright = (
                            last_char_rect.topleft[0] + newrect.width + 2,
            ...
            #     if ch not in up_vowel and ch not in down_vowel :
            # newrect.centery = newrect.centery + curve[i]
            #     newrect.centery = max(
            #                 newrect.height,
            #                 min(fsize[1] - newrect.height, newrect.centery + curve[i]))
            #     else:
            #         newrect.centery = max(fsize[1] - newrect.height, newrect.centery + curve[i])
    ...
    def old_render_curved(self, font, word_text):
        """
