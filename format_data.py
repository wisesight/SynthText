...
def run_generate_synth_text(all_fonts_path, font_familiy_path):
    ...
    with open(os.path.join(all_fonts_path, "fontlist.txt"), "w") as font_list_file:
        ...
        font_files_paths = [
            font_family_path.replace(all_fonts_path, "")
            for font_family_path in _font_files_paths
        ]
    ...
    while generate_run_status_code != 0:
        generate_run_status_code = os.system(
            f"python _gen.py --out_dir {os.path.join('results', os.path.basename(font_familiy_path))}"
        )
